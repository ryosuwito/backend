...
class IndividualApplicationForm(forms.Form):
    name = forms.CharField(required=False, label='Applicant Name*',  widget=forms.TextInput(attrs={
            'class':'form-control'}))
    univ = forms.CharField(required=False, label='University*',  widget=forms.TextInput(attrs={
            'class':'form-control'}))
    ...
    email = forms.EmailField(required=False, help_text="One email can only be used once.", label='Email*',  widget=forms.TextInput(attrs={
            'class':'form-control',
            'id': 'email'}))
...
class GroupApplicationForm(forms.Form):
    ...
    group = forms.ChoiceField(choices=GROUP_CHOICES, widget=forms.RadioSelect(attrs={'onclick': 'checkGroup();'}), label='Group*')
    
    group_name = forms.CharField(required=False, label='Group Name*',   widget=forms.TextInput(attrs={
            'class':'form-control',
            'id': 'groupNameId'}))
    ...
    def groupObjectFields(self):
        return [field for field in self if not field.is_hidden
               and field.name in ('group', 'group_name')]
